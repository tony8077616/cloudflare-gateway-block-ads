...
每個來源的取得有三種方式，前一種失敗才換下一種：

| 順位 | 方式 | 說明 |
|---|---|---|
| ① | 原網址直連 | 失敗原因是傳輸沒有完成、伺服器沒回應或忙碌（`000`、`403`、`408`、`425`、`429`、`5xx`）、內容不是純文字清單、上次有網域這次卻解析出 0 筆時，換 ② |
| ② | 同網址換連線參數（`--http1.1 -4`、較短的逾時） | 換 ③ 的條件與 ① 換 ② 相同 |
| ③ | 同網址改用 DoH 解析 DNS（`--doh-url https://1.1.1.1/dns-query`，逾時同 ②） | 擋得住：runner 的 DNS 解析故障、系統 DNS 導向的那個 CDN 節點故障或逾時、系統 DNS 回傳錯誤的位址。擋不住：原站所有節點都掛掉、原站依來源 IP 限流、1.1.1.1 本身連不上 |

② 與 ③ 整趟執行**合計**最多用 480 秒，用完之後失敗的來源不再嘗試 ② ③。

③ 的內容仍來自原站、同一個網址，跟 ① 的地位相同（ETag 照常記錄、移除照常進行）。
DoH 伺服器用 IP 字面值，runner 的 DNS 壞掉時仍連得到它（主機名形式本身就需要 DNS）；它的 TLS 憑證照常驗證，
解析器也只能把連線導到別的位址，原站的憑證驗證、內容檢查與 0 筆防護一道都沒有少。
DoH 請求會讀 `https_proxy`／`all_proxy` 這類代理環境變數；GitHub 託管的 runner 不設代理，
自架 runner 若有設代理，DoH 查詢也會經過那個代理。
...
三種方式都失敗的來源沿用前次狀態，它的網域這次不會被從 Gateway 移除。
...
合併後的來源清單是空的、**白名單或自訂封鎖清單讀取失敗**、**讀不到 Gateway 上現有的清單，或其中任何一份的成員**、
...
**每一份清單的「成員」讀取也是同一組道理**，而且這是三處裡唯一一個終點是刪除的：

| | 意思 | 行為 |
|---|---|---|
| 某一份清單**真的是空的** | 上一次的空清單刪除失敗，留下一個空殼 | 正常，照常處理 |
| 某一份清單的成員**讀不到** | 一次暫時性 5xx、速率限制、回應結構不對 | 中止，不上傳也不刪除 |

把第二種當成第一種，那份清單的約 1000 筆網域會被當成不存在而重新分配到其他槽位，
讀錯的那一份則因為「變成空的」被**送出 DELETE**。這兩件事各自都只是翻攪，但只要
重新分配之後有任何一份上傳失敗，那批網域就真的沒有被任何清單涵蓋 —— 而原本裝著
它們的那一份已經被刪掉了。

這一處的讀取跑在背景子行程裡，離開狀態沒有辦法傳回來，所以失敗是靠檔案回報的：
每一份讀不到的清單會在暫存目錄留下一個記著原因的檔案，全部讀完之後一次檢查。

**同一組道理的最後一處在來源端**，而且它不是中止，是**押後**：

| | 意思 | 行為 |
|---|---|---|
| 某個網域**真的從來源消失了** | 上游把它拿掉了，或被白名單／原生分類刷掉 | 正常，從 Gateway 移除 |
| 某個來源**這次沒抓到** | 上游暫時掛掉、逾時、HTTP 非 200 | 它獨有的網域這次不移除，押後 |

為什麼這裡選押後而不是中止：抓取失敗的來源會**沿用前次 checksum**（避免下次被誤判
成「內容變動」）。所以如果照著移除了，下一次它抓成功、內容又沒變時，閘門會判定
無變動而**略過整次同步** —— 那些網域要等到有別的來源變動才會被補回來。一次暫時性
的抓取失敗，換來一段沒有上限的解除封鎖。

分辨的依據是「本次的合併結果裡有沒有這個網域」：有，代表來源還看得到它，是我們
主動刷掉的，照常移除（**白名單因此不受影響**）；沒有，就可能只是那個來源沒抓到。

已知的取捨：你這次剛好把某個網域從自訂封鎖清單移除、而它又不在任何來源裡時，
這次的移除也會一起被押後，要等到所有來源都抓成功的那一次才生效。抓取失敗的來源
每一次都會列在 Job Summary 裡，所以這個狀態不會是無聲的。
