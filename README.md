...
目前規模：15 個來源合併出 **277,894** 個網域，扣掉白名單與原生分類涵蓋的部分後，
實際上傳 **223,146** 個網域、分佈在 **224 份** Gateway 清單。
...
| D1 每日讀取 | 5,000,000 列 | 每次同步約 136 列 | KV 快照取代全表掃描 |
...
KV namespace ID 不是機密，直接寫在 `sync.sh` 的設定區（`KV_NAMESPACE_ID`），
也可以用同名環境變數覆蓋。**留空就會整個停用 KV 快取**，退回讀 D1，功能不受影響。

D1 的資料表由 `sync.sh` 的 `ensure_schema()` 在第一次執行時自動建立，不需要手動下 SQL。
...
| 清單上傳失敗 | `./manage.sh failures` 看診斷紀錄，會依清單編號統計是否有特定清單反覆失敗 |
...
唯一會主動中止的情況是「最終清單是空的」，那通常代表所有來源都抓取失敗，
這時中止比清空 Gateway 清單安全。
